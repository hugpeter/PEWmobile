...
export function notasFetchData(ano, idColegio, idioma, cedula, bimestre, nivel, token) {
  return (dispatch, getState) => {
      ...
      if(nivel < 1 && idColegio != 800){
        ...
        .then(json => {
          ...
          notas = new Array();
          currentIdArea = 0;
          todoNotas.forEach((nota) => {
            var area = {};

            if(nota.idarea != currentIdArea){
              area.idarea = nota.idarea;
              area.area = nota.area;

              areas.push(area);
              currentIdArea = nota.idarea;
            }
          });

          notas.push(todoNotas);
          notas.push(areas);
